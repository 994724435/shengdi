...
class PhotoController extends CommonController {
	public function index(){
		...
		if(IS_POST){
            ...
            if($res){
               echo "<script>window.location.href='/index.php/Adminen2/Photo/index';</script>";exit;
            }else{
                echo "<script>alert('删除失败');window.location.href='/index.php/Adminen2/Photo/index';</script>";exit;
            }
        }
		$data = array(
				'bp_id'=>$bp_id,
    ...
    public function add(){
    	...
    	if(IS_POST){
    		...
    		if(!$info) {// 上传错误提示错误信息
                ...
                $res = $banner->add($arr);
                if($res){
                	echo "<script>window.location.href='/index.php/Adminen2/Photo/index';</script>";
                }else{
