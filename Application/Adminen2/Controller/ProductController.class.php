...
class ProductController extends CommonController {
    public function index(){
        ...
        //数据
        if($q){
            $news=$product->join('`ast_procateen2` ON ast_producten2.pro_cate_id = ast_procateen2.cate_id')->where(array('pro_name'=>array('like',"%$q%")))->order('pro_addtime DESC')->limit($art,$limit)->select();
        }else{
            if($sid){
                $news=$product->join('`ast_procateen2` ON ast_producten2.pro_cate_id = ast_procateen2.cate_id')->where(array('pro_cate_id'=>$sid))->order('pro_addtime DESC')->limit($art,$limit)->select();
            }else{
                if($cid){
                    $ids=$procate->field('cate_id')->where(array('cate_pid'=>$cid))->select();
                    $idr=array(0=>$cid);
                    ...
                    $news=$product->join('`ast_procateen2` ON ast_producten2.pro_cate_id = ast_procateen2.cate_id')->where(array('pro_cate_id'=>array('IN',$idr)))->order('pro_addtime DESC')->limit($art,$limit)->select();
                }else{
                    $news=$product->join('`ast_procateen2` ON ast_producten2.pro_cate_id = ast_procateen2.cate_id')->order('pro_addtime DESC')->limit($art,$limit)->select();
                }
            }
        }
        $comment = M('Procomment2');
        $ment = $comment->select();
    ...
    public function delete(){
        ...
        if($date){
            echo "<script>window.location.href='/index.php/Adminen2/Product/index';</script>";exit;
        }else{
            echo "<script>alert('删除失败');window.location.href='/index.php/Adminen2/Product/index';</script>";exit;
        }
    }
    //产品批量删除
    public function formDelete(){       
        ...
        if($aid){           //如果$aid有值
            ...
            echo "<script>window.location.href='/index.php/Adminen2/Product/index';</script>";exit;      //如果$aid没值，直接跳转到产品首页
        }
    }
    ...
    public function editor(){
        ...
        $pro=$product->join('`ast_procateen2` ON ast_producten2.pro_cate_id = ast_procateen2.cate_id')->where(array('pro_id'=>$aid))->find();
        // dump($pro);die;
            ...
            if(!$res){
                ...
                if($_FILES['thumb']['name']){
                    ...
                    if(!$info) {// 上传错误提示错误信息
                        ...
                        echo "<script>window.location.href='/index.php/Adminen2/Product/index';</script>";
                    }
                }else{
                    echo "<script>window.location.href='/index.php/Adminen2/Product/index';</script>";
                }
    ...
    public function fir_class(){    //一级分类添加
        if(IS_POST){
            ...
            $res = $cate->add($data);
            if($res){
                echo "<script>window.location.href='/index.php/Adminen2/Product/sort';</script>";
            }
        }
    }
    ...
    public function cateDelete(){
        ...
        if($cid){
            ...
            if($data){
                    echo "<script>window.location.href='/index.php/Adminen2/Product/sort';</script>";exit;
                }else{
                    echo "<script>alert('删除失败');window.location.href='/index.php/Adminen2/Product/sort';</script>";exit;
                }
