...
class ProductController extends CommonController {
    public function index(){
        ...
        $q=I('get.q','');//搜索
    	$product=M('Producten5');
    	$procate=M('Procateen5');
        ...
        //数据
        if($q){
            $news=$product->join('`ast_Procateen5` ON ast_Producten5.pro_cate_id = ast_Procateen5.cate_id')->where(array('pro_name'=>array('like',"%$q%")))->order('pro_addtime DESC')->limit($art,$limit)->select();
        }else{
            if($sid){
                $news=$product->join('`ast_Procateen5` ON ast_Producten5.pro_cate_id = ast_Procateen5.cate_id')->where(array('pro_cate_id'=>$sid))->order('pro_addtime DESC')->limit($art,$limit)->select();
            }else{
                if($cid){
                    $ids=$procate->field('cate_id')->where(array('cate_pid'=>$cid))->select();
                    $idr=array(0=>$cid);
                    ...
                    $news=$product->join('`ast_Procateen5` ON ast_Producten5.pro_cate_id = ast_Procateen5.cate_id')->where(array('pro_cate_id'=>array('IN',$idr)))->order('pro_addtime DESC')->limit($art,$limit)->select();
                }else{
                    $news=$product->join('`ast_Procateen5` ON ast_Producten5.pro_cate_id = ast_Procateen5.cate_id')->order('pro_addtime DESC')->limit($art,$limit)->select();
                }
            }
        }
        $comment = M('Procommenten5');
        $ment = $comment->select();
    ...
    public function editor(){
        ...
        $pro=$product->join('`ast_Procateen5` ON ast_Producten5.pro_cate_id = ast_Procateen5.cate_id')->where(array('pro_id'=>$aid))->find();
        // dump($pro);die;
