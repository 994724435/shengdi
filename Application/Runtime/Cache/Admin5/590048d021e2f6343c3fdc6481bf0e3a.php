...
<div class="top">
  ...
  <div class="welc">您好，欢迎您登录网站后台管理平台</div>
   <div class="welc"><a href="/index.php/Adminen5/Index/index" style="color: red;">切换到英文版</a></div>
</div>
<div class="clear"></div>
<div class="leftfix">
...
<!--添加文章-->
<div class="rightfix">
  <a class="home1"></a><a class="home_pass">招聘管理</a><a href="<?php echo U('/index.php/Admin5/Join/index');?>" class="home_pass">招聘列表</a><a class="home_now">招聘添加</a>
  <div class="clear"></div>
  <div style="width:1072px;margin-left:5px;">
    <form action="<?php echo U('/index.php/Admin5/Join/add');?>" method='post' enctype="multipart/form-data">
...
$("#cate").change(function(){
  var pid=$(this).val(); if(pid){$.post('/index.php/Admin5/Article/cateAjax',{'pid':pid},function(data){$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } },'json'); }else{$('#scate').empty().append('<option value="">未分类</option>'); } });

//新建分类
$(".newsorttext").click(function(){
  var sort = $(".newsort").val(); var cid = $("#cate").val(); $.post('/index.php/Admin5/Article/sortAjax',{'sort':sort,'cid':cid},function(data){if(!cid){$('#cate').empty().append('<option value="">未分类</option>'); $('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#cate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } }else{$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } } },'json'); });
...
<div class="changepower block changePermissions_block">
  	...
  	<form action="<?php echo U('/index.php/Admin5/Index/formDelete');?>" method="post">
      ...
      <div id="demo">
	  	<?php if(is_array($user)): foreach($user as $key=>$val): ?><div class="subbox">
		      	...
		      	<div class="bkuang4"><a class="a1" href="<?php echo U('/index.php/Admin5/User/del','admin_id='.$val['admin_id']);?>">删除</a> /  <a adminid="<?php echo ($val["admin_id"]); ?>" class="a2  show_block setPermissions_block">权限设置</a> /  <a class="a3  changePassword_block show_block" admID="<?php echo ($val["admin_id"]); ?>">修改资料</a></div>
	    	</div>
...
<div class="changepower2 block setPermissions_block">
    ...
    <form action="<?php echo U('/index.php/Admin5/Index/add');?>" method="post" id="form">
...
<div class="changepassword  changePassword_block block">
	...
	<form action="<?php echo U('/index.php/Admin5/Index/change');?>" method="post">
...
<div class="logout friendShip2_notice_block block">
  	...
  	<a class="makesure" href="<?php echo U('/index.php/Admin5/User/logout');?>">确定退出</a><a class="cancle">取消</a>
</div>
...
<script>
  $('#account').blur(function(){
      ...
      $.post('/index.php/Admin5/index/account_ajax',{'account':account},function(num){
          if(num == 0){
            $('#msg').text('用户名不存在');
          }else{
            $('#msg').empty();
      ...
      $('#psd0').blur(function(){
          var psd0 = $(this).val();
          $.post('/index.php/Admin5/Index/ajax',{'psd':psd0,'acc':account},function(data){
            if(data == 0){
                $('#msg0').text('密码错误');
            }else{
                $('#msg0').empty();
    ...
    $('.a3').click(function(){
      ...
      $.post('/index.php/Admin5/User/admin',{'admin_id':admID},function(data){
  ...
  function findstr(str,arr){
    ...
    $('#next').click(function(){
      var nuna = $('#acc').val();
      var pswd = $('#psd3').val();
      $.post('/index.php/Admin5/Index/prev_add',{'nuna':nuna,'pswd':pswd},function(data){
  ...
  $('.show_block').click(function(){
      ...
      $.post('/index.php/Admin5/User/admin',{'admin_id':admin_id},function(data){
        ...
        if(data){
          $('#form').attr('action',"<?php echo U('/index.php/Admin5/Index/update');?>");
        };
  ...
  $('#cancle').click(function(){
    ...
    $('.num').click(function(){
      ...
      var page = $(this).text();
      if(page==endPage){
        $.post('/index.php/Admin5/Common/admin_page',{'page':endPage},function(data){
        ...
        $.post('/index.php/Admin5/Common/admin_page',{'page':page},function(data){
        for(var i=0;i<data.user.length;i++){
        ...
        if($('.current').text()>3 && $('.current').text()<all){
          ...
          if(t){
            ...
            $('.first').click(function(){
              $.post('/index.php/Admin5/Common/admin_page',{'page':1},function(data){
                for(var i=0;i<data.user.length;i++){
                  $('#demo .subbox:eq('+i+') .bkuang1').show().attr('name',"idarr['+data.user[i]['admin_id']+']");
                  $('#demo .subbox:eq('+i+') .bkuang2').show().text(data.user[i]['admin_username']);
                  if(data.user[i]['admin_level'] == 1){
