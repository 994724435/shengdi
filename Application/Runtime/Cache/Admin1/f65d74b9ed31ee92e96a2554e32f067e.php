...
<div class="rightfix">
  ...
  <div class="clear"></div>
 	<div class="article_edit_list" style="width:1086px ;">
 		<form action="<?php echo U('/index.php/Admin1/Product/editor',array('aid'=>$aid));?>" method='post' enctype="multipart/form-data">
    <div class="article_edit_list">
      <!-- 产品名称 -->    
      <div class="edit_title"><div class="col_left f">产品名</div><div class="col_right f"><input type="text" name="title" class="title_text" value="<?php echo ($pro["pro_name"]); ?>"/></div><div class="clear"></div></div> <div class="clear"></div>
      <!-- 产品分类 -->
      <div class="choose_sort same_height"> <div class="col_left f">选择分类</div> <div class="col_right f"> <select class="sort f" name="cate" id="cate"> <option value="" >未分类</option> <?php if(is_array($cate)): foreach($cate as $key=>$value): ?><option value="<?php echo ($value["cate_id"]); ?>" <?php if($pid == 0){if($pro['pro_cate_id'] == $value['cate_id']){echo "selected=selected" ;}}else{if($pid==$value['cate_id']){echo "selected=selected" ;}} ?>><?php echo ($value["cate_name"]); ?></option><?php endforeach; endif; ?> </select> <select class="sort f" name="scate" id="scate" style="margin-left:25px;"> <option value="" <?php if($pro['pro_cate_id'] != $val['cate_id']){echo "selected=selected";} ?>>未分类</option> <?php if(is_array($scate)): foreach($scate as $key=>$val): ?><option value="<?php echo ($val["cate_id"]); ?>" <?php if($pro['pro_cate_id'] == $val['cate_id']){echo "selected=selected";} ?>><?php echo ($val["cate_name"]); ?></option><?php endforeach; endif; ?> </select> <div class="f"><input type="text" class="newsort" style="width:120px;"><label class="newsorttext">新建分类</label></div> <div class="clear"></div> </div> </div> <div class="clear"></div>
      <!-- 产品缩略图 -->
      <div class="thumbnail"> <div class="col_left f">缩略图</div> <div class="col_right f"> <div class="img_containner f"> <img id="imghead" src='<?php echo ($pro["pro_thumb"]); ?>'>  </div> <div class="add_img_c"> <div class="fileimg"><input type="file" onchange="previewImage(this)" name="thumb"/></div> </div> </div> <div class="clear"></div> </div> <div class="clear"></div>
      <!-- 产品是否显示 -->
      <div class="submit_person same_height"><div class="col_left f">是否显示</div><div class="col_right f"><?php if($pro['pro_isshow'] == 1): ?><input type="radio" name="isshow" value="1" id="" checked='checked'>显示<input type="radio" name="isshow" id="" value="0">不显示<?php else: ?><input type="radio" name="isshow" value="1" id="">显示<input type="radio" name="isshow" value="0" checked='checked' id="">不显示<?php endif; ?></div><div class="clear"></div></div> <div class="clear"></div>
      <!-- 产品上架时间 -->
      <div class="public same_height"><div class="col_left f">上架时间</div><div class="col_right f"><input type="text" name="time" class="title_text" readonly="true" value="<?php echo (date("Y-m-d",$pro["pro_addtime"])); ?>" /></div><div class="clear"></div></div> <div class="clear"></div>
      <!-- 产品来源 -->
      <div class="source same_height"><div class="col_left f">来源</div><div class="col_right f"><input type="text" class="title_text" name="source" value="<?php echo ($pro["pro_from"]); ?>" /></div><div class="clear"></div></div> <div class="clear"></div>
      <!-- 产品价格 -->
      ...
      <!-- 产品简介 -->
      <div class="digest same_height"><div class="col_left f">摘要</div><div class="col_right f"><input type="text" name="summary" value="<?php echo ($pro["pro_summary"]); ?>" class="title_text Summary" /></div> </div><div class="clear"></div>
      <!-- 产品详细介绍 -->
      <div class="article_detail"><div class="col_left f d_bottom">产品介绍</div><div class="col_right f d_bottom"><div class="detail_edit"> <?php echo $htmlData; ?> <textarea name="content1" class="textarea"><?php echo ($pro["pro_introduction"]); echo htmlspecialchars($htmlData); ?></textarea> </div></div></div> <div class="clear"></div>
      <!-- 提交或者取消 -->
...
//分类AJAX
$("#cate").change(function(){var pid=$(this).val(); if(pid){$.post('/index.php/Admin1/Product/cateAjax',{'pid':pid},function(data){$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } },'json'); }else{$('#scate').empty().append('<option value="">未分类</option>'); } });

//新建分类
$(".newsorttext").click(function(){var sort = $(".newsort").val(); var cid = $("#cate").val(); $.post('/index.php/Admin1/Product/sortAjax',{'sort':sort,'cid':cid},function(data){if(!cid){$('#cate').empty().append('<option value="">未分类</option>'); $('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#cate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } }else{$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } } },'json'); });
