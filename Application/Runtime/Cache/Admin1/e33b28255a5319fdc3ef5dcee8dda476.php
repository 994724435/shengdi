    ...
    <div class="title">
      ...
      <div id="dd" class="wrapper-dropdown-3" tabindex="1">
        <span style="color:#e2e1e1;"><?php if($cid != 0): echo ($cate["cate_name"]); else: ?>未分类<?php endif; ?></span>
      	<ul class="dropdown">
          <li><a onclick="javascript:window.location.href='<?php echo U('/index.php/Admin1/Product/index');?>'">未分类</a></li>
          <?php if(is_array($acate)): foreach($acate as $key=>$val): ?><li><a onclick="javascript:window.location.href='<?php echo U('/index.php/Admin1/Product/index',array('cid'=>$val['cate_id']));?>'"><?php echo ($val["cate_name"]); ?></a></li><?php endforeach; endif; ?>
      	</ul>
      </div>
      <div id="aa" class="wrapper-dropdown-3" tabindex="1">
        ...
        <?php if($cid != 0): ?><ul class="dropdown">
            <li><a onclick="javascript:window.location.href='<?php echo U('/index.php/Admin1/Product/index',array('cid'=>$cate['cate_id']));?>'">未分类</a></li>
            <?php if(is_array($scate)): foreach($scate as $key=>$val): ?><li><a onclick="javascript:window.location.href='<?php echo U('/index.php/Admin1/Product/index',array('cid'=>$cate['cate_id'],'sid'=>$val['cate_id']));?>'"><?php echo ($val["cate_name"]); ?></a></li><?php endforeach; endif; ?>
          </ul>
        <?php else: ?>
          <ul class="dropdown">
            <li><a onclick="javascript:window.location.href='<?php echo U('/index.php/Admin1/Product/index',array('cid'=>$cate['cate_id']));?>'">未分类</a></li>
      ...
      <form class="allen" action="<?php echo U('/index.php/Admin1/Product/index');?>"> 
      ...
      <table>
        ...
        <?php if(is_array($news)): foreach($news as $key=>$value): ?><tr class="table_list">
          ...
          <?php if($value['pro_isshow'] == 1): ?><a class="aa1" href="javascript:;" isshow="1" artID="<?php echo ($value["pro_id"]); ?>" style="display:inline">隐藏</a><a href="javascript:;" class="aa4" isshow="0" artID="<?php echo ($value["pro_id"]); ?>" style="display:none" >显示</a>
              <?php else: ?><a class="aa1" href="javascript:;" isshow="0" artID="<?php echo ($value["pro_id"]); ?>" style="display:none">隐藏</a><a href="javascript:;" class="aa4" isshow="0" artID="<?php echo ($value["pro_id"]); ?>" style="display:inline" >显示</a><?php endif; ?> /  <a href="<?php echo U('/index.php/Admin1/Product/editor',array('aid'=>$value['pro_id']));?>" class="aa2">编辑</a> /  <a href="<?php echo U('/index.php/Admin1/Product/delete',array('aid'=>$value['pro_id']));?>" class="aa3">删除</a>/<a class="pinglun" href="<?php echo U('/index.php/Admin1/Product/comment',array('pid'=>$value['pro_id']));?>"><?php if($value['mentNum']){echo "评论(".$value['mentNum'].")";}else{echo "评论(0)";} ?></a></td>
      ...
      <script type="text/javascript">
        ...
        if(adm_power.indexOf('产品列表') === -1){
          ...
          $('.order').blur(function(){
            ...
            $.post('/index.php/Admin1/Product/order',{'oldOrder':oldOrder,'order':order,'id':id},function(data){
              // console.log(data);
              if(!data){
                return false;
              }  
          ...
          $('.aa4').click(function(){
            ...
            var id = $(this).attr('artID');
            $.post('/index.php/Admin1/Product/isshow',{'show':show,'id':id},function(data){
              if(data == false){
                return false;
              }
            },'json')
