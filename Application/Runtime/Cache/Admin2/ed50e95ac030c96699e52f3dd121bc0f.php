...
<div class="rightfix">
  ...
  <div class="clear"></div>
 	<div class="article_edit_list" style="width:1086px ;">
 		<form action="<?php echo U('/index.php/Admin2/Product/editor',array('aid'=>$aid));?>" method='post' enctype="multipart/form-data">
...
//分类AJAX
$("#cate").change(function(){var pid=$(this).val(); if(pid){$.post('/index.php/Admin2/Product/cateAjax',{'pid':pid},function(data){$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } },'json'); }else{$('#scate').empty().append('<option value="">未分类</option>'); } });

//新建分类
$(".newsorttext").click(function(){var sort = $(".newsort").val(); var cid = $("#cate").val(); $.post('/index.php/Admin2/Product/sortAjax',{'sort':sort,'cid':cid},function(data){if(!cid){$('#cate').empty().append('<option value="">未分类</option>'); $('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#cate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } }else{$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } } },'json'); });
