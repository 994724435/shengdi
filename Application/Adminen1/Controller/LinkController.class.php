...
class LinkController extends CommonController {
	public function pic(){
		...
		if($_FILES['thumb']['name']){
			 ...
			 if($res){
				echo "<script>window.location.href='/index.php/Adminen1/Link/intro';</script>";
			}else{
				echo "<script>window.location.href='/index.php/Adminen1/Link/intro';</script>"; 
			}	
		...
		if($_POST['title']){
		  ...
		  $res  =$words->where(array('w_id'=>$_GET['id']))->save($data);
		    if($_FILES['thumb']['name']){
			  ...
			  if($res||$res1){
			  	   echo "<script>window.location.href='/index.php/Adminen1/Link/words_link';</script>";
			  }else{
			  	 echo "<script>window.location.href='/index.php/Adminen1/Link/words_link';</script>";
			  }
		}
