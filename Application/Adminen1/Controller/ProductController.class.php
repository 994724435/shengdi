...
class ProductController extends CommonController {
    public function index(){
        ...
        $q=I('get.q','');//搜索
    	$product=M('Producten1');
    	$procate=M('Procateen1');
        ...
        //数据
        if($q){
            $news=$product->join('`ast_procateen1` ON ast_producten1.pro_cate_id = ast_procateen1.cate_id')->where(array('pro_name'=>array('like',"%$q%")))->order('pro_addtime DESC')->limit($art,$limit)->select();
        }else{
            if($sid){
                $news=$product->join('`ast_procateen1` ON ast_producten1.pro_cate_id = ast_procateen1.cate_id')->where(array('pro_cate_id'=>$sid))->order('pro_addtime DESC')->limit($art,$limit)->select();
            }else{
                if($cid){
                    $ids=$procate->field('cate_id')->where(array('cate_pid'=>$cid))->select();
                    $idr=array(0=>$cid);
                    ...
                    $news=$product->join('`ast_procateen1` ON ast_producten1.pro_cate_id = ast_procateen1.cate_id')->where(array('pro_cate_id'=>array('IN',$idr)))->order('pro_addtime DESC')->limit($art,$limit)->select();
                }else{
                    $news=$product->join('`ast_procateen1` ON ast_producten1.pro_cate_id = ast_procateen1.cate_id')->order('pro_addtime DESC')->limit($art,$limit)->select();
                }
            }
        }
        $comment = M('Procommenten1');
        $ment = $comment->select();
    ...
    public function delete(){
        ...
        if($date){
            echo "<script>window.location.href='/index.php/Adminen1/Product/index';</script>";exit;
        }else{
            echo "<script>alert('删除失败');window.location.href='/index.php/Adminen1/Product/index';</script>";exit;
        }
    }
    //产品批量删除
    public function formDelete(){       
        ...
        if($aid){           //如果$aid有值
            ...
            echo "<script>window.location.href='/index.php/Adminen1/Product/index';</script>";exit;      //如果$aid没值，直接跳转到产品首页
        }
    }
    ...
    public function editor(){
        ...
        $pro=$product->join('`ast_procateen1` ON ast_producten1.pro_cate_id = ast_procateen1.cate_id')->where(array('pro_id'=>$aid))->find();
        // dump($pro);die;
            ...
            if(!$res){
                ...
                if($_FILES['thumb']['name']){
                    ...
                    if(!$info) {// 上传错误提示错误信息
                        ...
                        echo "<script>window.location.href='/index.php/Adminen1/Product/index';</script>";
                    }
                }else{
                    echo "<script>window.location.href='/index.php/Adminen1/Product/index';</script>";
                }
    ...
    public function fir_class(){    //一级分类添加
        if(IS_POST){
            ...
            $res = $cate->add($data);
            if($res){
                echo "<script>window.location.href='/index.php/Adminen1/Product/sort';</script>";
            }
        }
    }
    ...
    public function cateDelete(){
        ...
        if($cid){
            ...
            if($data){
                    echo "<script>window.location.href='/index.php/Adminen1/Product/sort';</script>";exit;
                }else{
                    echo "<script>alert('删除失败');window.location.href='/index.php/Adminen1/Product/sort';</script>";exit;
                }
