...
class articleController extends CommonController {
    public function index(){
    	...
    	if($q){
            ...
            $news=$article->join('`ast_artcate3` ON ast_article3.art_cateid = ast_artcate3.cate_id')->where(array('art_title'=>array('like',"%$q%")))->order('art_addtime DESC')->limit($art,$limit)->select();
    	}else{
            if($sid){
                $count=$article->where(array('art_cateid'=>$sid))->count();
                $news=$article->join('`ast_artcate3` ON ast_article3.art_cateid = ast_artcate3.cate_id')->where(array('art_cateid'=>$sid))->order('art_addtime DESC')->limit($art,$limit)->select();
            }else{
                if($cid){
                    ...
                    $news=$article->join('`ast_artcate3` ON ast_article3.art_cateid = ast_artcate3.cate_id')->where(array('art_cateid'=>array('IN',$idr)))->order('art_addtime DESC')->limit($art,$limit)->select();
                }else{
                    $count=$article->count();
                    $news=$article->join('`ast_artcate3` ON ast_article3.art_cateid = ast_artcate3.cate_id')->order('art_addtime DESC')->limit($art,$limit)->select();
                }
            }
    		
    	}
        $show = list_page($current,$limit,$count,$purl);
        $comment = M('Artcomment3');
        $ment = $comment->select();
    ...
    public function delete(){
        ...
        if($date){
            echo "<script>window.location.href='/index.php/Admin3/article/index';</script>";exit;
        }else{
            echo "<script>alert('删除失败');window.location.href='/index.php/Admin3/article/index';</script>";exit;
        }
    }
    ...
    public function editor(){
        ...
        $art=$article->join('`ast_artcate3` ON ast_article3.art_cateid = ast_artcate3.cate_id')->where(array('art_id'=>$aid))->find();
            ...
            if(!$res){
                ...
                if($_FILES['thumb']['name']){
                    ...
                    if(!$info) {// 上传错误提示错误信息
                        ...
                        echo "<script>window.location.href='/index.php/Admin3/Article/index';</script>";
                    }
                }else{
                    echo "<script>window.location.href='/index.php/Admin3/Article/index';</script>";
                }
    ...
    public function fir_class(){    //一级分类添加
        if(IS_POST){
            ...
            $res = $cate->add($data);
            if($res){
                echo "<script>window.location.href='/index.php/Admin3/Article/sort';</script>";
            }
        }
    }
    ...
    public function cateDelete(){
        ...
        if($cid){
            ...
            if($data){
                    echo "<script>window.location.href='/index.php/Admin3/article/sort';</script>";exit;
                }else{
                    echo "<script>alert('删除失败');window.location.href='/index.php/Admin3/article/sort';</script>";exit;
                }
    ...
    public function commentDel(){
            ...
            // print_r($artid);die;
            if($res){
                echo "<script>window.location.href='/index.php/Admin3/Article/comment/aid/".$artid['cmt_art_id']."'.html;</script>";
            }else{
                echo "<script>alert('删除失败');window.location.href='/index.php/Admin3/Article/comment';</script>";die;
            }
    }
