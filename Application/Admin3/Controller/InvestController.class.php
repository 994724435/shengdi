...
class InvestController extends CommonController {
    ...
    public function delete(){
    	$id = I('get.id');
		if($id==1||$id==2){
			 echo "<script>alert('栏目项不能删除');window.location.href='/index.php/Admin3/Invest/index';</script>";exit;
		}
        $invest =M('Invest3');
        ...
        if($date){
            echo "<script>window.location.href='/index.php/Admin3/Invest/index';</script>";exit;
        }else{
	...
	public function add(){
		...
		if($res||$result){
			echo "<script>window.location.href='/index.php/Admin3/Invest/index';</script>";
		}else{
			echo "<script>window.location.href='/index.php/Admin3/Invest/index';</script>";
		}			
			 }	
